...
class UserController extends BaseController
{
    ...
    public function editAction($id)
    {
        ...
        //Set the user id on a session bag to use it in the updateAction method
    ...
    public function updateAction()
    {
        ...
        $manager    = $this->getDI()->get('core_user_manager');
        $user_id  = $this->persistent->get('id');
        $user     = $manager->findFirstById($user_id);
        $form       = $manager->getForm($user);

        if ($form->isValid($this->request->getPost())) {
            try {
                $manager = $this->getDI()->get('core_user_manager');
                $manager->update(array_merge($this->request->getPost(), ['id' => $user_id]));
